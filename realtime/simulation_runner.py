...
def run_simulation(realtime_simulator, duration_hours=24, update_interval_minutes=15):
    ...
    """
    print(f"Starting real-time simulation for {realtime_simulator.symbol} for {duration_hours} hours")
    ...
    while datetime.now() < end_time:
        try:
            ...
            # Get latest data
            latest_df = realtime_simulator.get_latest_data(lookback_candles=2)  # Just get the latest candles
            ...
            if tp_sl_result:
                ...
                try:
                    if realtime_simulator.ml_manager:
                        ml_signal, ml_confidence = realtime_simulator.ml_manager.get_ml_signal(
                ...
                if signal != 0:
                    ...
                    trade_result = realtime_simulator.execute_trade(signal, current_price, latest_candle['timestamp'])
                    ...
                    if trade_result:
                        ...
                        if signal == 1:  # BUY signal
                            stop_loss_price = current_price * (1 - realtime_simulator.simulator.stop_loss_pct)
                            take_profit_price = current_price * (1 + realtime_simulator.simulator.take_profit_pct)
                        else:  # SELL signal
                            stop_loss_price = current_price * (1 + realtime_simulator.simulator.stop_loss_pct)
                            take_profit_price = current_price * (1 - realtime_simulator.simulator.take_profit_pct)
            ...
            print(f"Current balance: ${realtime_simulator.simulator.current_balance:.2f}")
            if realtime_simulator.simulator.position:
                position_value = realtime_simulator.simulator.position_size * current_price
                profit_loss = 0
                if realtime_simulator.simulator.position == 'long':
                    profit_loss = realtime_simulator.simulator.position_size * (current_price - realtime_simulator.simulator.entry_price)
                else:  # short
                    profit_loss = realtime_simulator.simulator.position_size * (realtime_simulator.simulator.entry_price - current_price)
                
                print(f"Current position: {realtime_simulator.simulator.position.upper()}")
                print(f"Entry price: ${realtime_simulator.simulator.entry_price:.2f}")
                print(f"Position size: {realtime_simulator.simulator.position_size:.6f} units")
                ...
                if realtime_simulator.simulator.position:
                    ...
                    profit_pct = 0
                    if realtime_simulator.simulator.position == 'long':
                        profit_pct = (current_price - realtime_simulator.simulator.entry_price) / realtime_simulator.simulator.entry_price * 100
                    else:  # short
                        profit_pct = (realtime_simulator.simulator.entry_price - current_price) / realtime_simulator.simulator.entry_price * 100
    ...
    print(f"Profit/Loss: ${realtime_simulator.simulator.current_balance - realtime_simulator.initial_investment:.2f}")
    print(f"Return: {((realtime_simulator.simulator.current_balance - realtime_simulator.initial_investment) / realtime_simulator.initial_investment) * 100:.2f}%")
    
    # Close any open positions
    if realtime_simulator.simulator.position:
        print("Closing open position...")
        current_price = float(realtime_simulator.client.get_symbol_ticker(symbol=realtime_simulator.symbol)['price'])
        realtime_simulator.simulator.close_position(current_price, datetime.now(), "simulation_end")
    ...
    return {
        ...
        'profit_loss': realtime_simulator.simulator.current_balance - realtime_simulator.initial_investment,
        'return_pct': ((realtime_simulator.simulator.current_balance - realtime_simulator.initial_investment) / realtime_simulator.initial_investment) * 100,
