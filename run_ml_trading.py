...
def parse_arguments():
    ...
    # ML parameters
    parser.add_argument("--model-type", type=str, default="ensemble", choices=["rf", "gb", "ensemble"], 
    ...
    parser.add_argument("--optimize", action="store_true", 
                        help="Optimize ML model hyperparameters")
    parser.add_argument("--force-retrain", action="store_true", 
                        help="Force retraining of ML model even if it exists")
    parser.add_argument("--ml-confidence", type=float, default=0.6, 
    ...
    parser.add_argument("--test-mode", action="store_true", 
                        help="Run in test mode without real trades")
    parser.add_argument("--backtest-first", action="store_true", 
                        help="Run backtest before live trading")
    parser.add_argument("--compound", action="store_true", 
                        help="Enable compound interest")
    
    # Take profit and stop loss settings
    parser.add_argument("--dynamic-tp", action="store_true", 
                        help="Use dynamic take profit based on market conditions")
    parser.add_argument("--fixed-tp", type=float, default=0, 
                        help="Fixed take profit percentage (0 to disable)")
    parser.add_argument("--fixed-sl", type=float, default=0, 
...
def main():
    ...
    if args.backtest_first or args.force_retrain:
        ...
        if backtest_results:
            ...
            if 'max_drawdown_pct' not in backtest_results and 'max_drawdown' in backtest_results:
            ...
            if 'sharpe_ratio' not in backtest_results:
                ...
                if backtest_results.get('max_drawdown_pct', 0) > 0:
                    backtest_results['sharpe_ratio'] = backtest_results.get('total_return_pct', 0) / backtest_results.get('max_drawdown_pct', 1)
                else:
